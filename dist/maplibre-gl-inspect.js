                        ...
                        (function (root, factory) {
                            ...
                            function pickHue(options) {
                                if (colorRanges.length > 0) {
                                    var hueRange = getRealHueRange(options.hue) var hue = randomWithin(hueRange)  //Each of colorRanges.length ranges has a length equal approximatelly one step
                                            var step = (hueRange[1] - hueRange[0]) / colorRanges.length var j = parseInt((hue - hueRange[0]) / step)  //Check if the range j is taken
                            ...
                            function randomWithin(range) {
                                if (seed === null) {
                                    ...
                                    var golden_ratio = 0.618033988749895 var r = Math.random() r += golden_ratio r %= 1 return Math.floor(range[0] + r * (range[1] + 1 - range[0]));
                                } else {
                            ...
                            function getRealHueRange(colorHue) {
                                if (!isNaN(colorHue)) {
                                    ...
                                    if (colorDictionary[colorHue]) {
                                        ...
                                        var hue = HexToHSB(colorHue)[0] return getColorInfo(hue).hueRange
                                    }
                                }

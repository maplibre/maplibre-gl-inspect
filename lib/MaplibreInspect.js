...
function isInspectStyle(style) {
  return style.metadata && style.metadata['maplibre-gl-inspect:inspect'];
}

function markInspectStyle(style) {
  return Object.assign(style, {
    metadata: Object.assign({}, style.metadata, {
      'maplibre-gl-inspect:inspect': true
    })
  });
}
...
function notifyVersion(maplibregl) {
  var versions = maplibregl.version.split('.').map(parseFloat);
  if (versions[0] < 1 && versions[1] < 29) {
    console.error('MaplibreInspect only supports Maplibre GL JS >= v0.29.0. Please upgrade your Maplibre GL JS version.');
  }
}
...
function MaplibreInspect(options) {
  ...
  if (window.maplibregl) {
    notifyVersion(window.maplibregl);

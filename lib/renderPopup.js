...
function renderProperty(propertyName, property) {
  return '<div class="maplibre-gl-inspect_property">' +
    '<div class="maplibre-gl-inspect_property-name">' + propertyName + '</div>' +
    '<div class="maplibre-gl-inspect_property-value">' + displayValue(property) + '</div>' +
    '</div>';
}

function renderLayer(layerId) {
  return '<div class="maplibre-gl-inspect_layer">' + layerId + '</div>';
}
...
function renderFeatures(features) {
  return features.map(function (ft) {
    return '<div class="maplibre-gl-inspect_feature">' + renderProperties(ft) + '</div>';
  }).join('');
}

function renderPopup(features) {
  return '<div class="maplibre-gl-inspect_popup">' + renderFeatures(features) + '</div>';
}
